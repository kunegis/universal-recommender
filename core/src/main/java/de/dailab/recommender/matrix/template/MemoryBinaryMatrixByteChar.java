...
final class MemoryBinaryMatrixByteChar
    implements Matrix
{
	public MemoryBinaryMatrixByteChar(int m, int n)
	{
		rows = new char[m][];
		cols = new byte[n][];
		rowCounts = new int[m];
	...
	private static void setA(char vectors[][], int counts[], int i, int j)
	{
		assert i >= 0;
		assert j >= 0;
	...
	private static void setB(byte vectors[][], int counts[], int i, int j)
	{
		assert i >= 0;
		assert j >= 0;
		assert i < vectors.length;
	...
	private static Iterable <Entry> iterableA(char vectors[][], int i, final int count)
	{
	...
	private static Iterable <Entry> iterableB(byte vectors[][], int i, final int count)
	{
		assert i >= 0 && i < vectors.length;
	...
	private static double getA(byte vector[], int count, int i)
	{
		assert i >= 0;
	...
	private static double getB(char vector[], int count, int i)
	{
		assert i >= 0;
