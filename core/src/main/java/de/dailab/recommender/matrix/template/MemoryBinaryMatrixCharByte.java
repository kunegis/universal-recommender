...
final class MemoryBinaryMatrixCharByte
    implements Matrix
{
	public MemoryBinaryMatrixCharByte(int m, int n)
	{
		rows = new byte[m][];
		cols = new char[n][];
		rowCounts = new int[m];
	...
	private static void setA(byte vectors[][], int counts[], int i, int j)
	{
		assert i >= 0;
		assert j >= 0;
		...
		vectors[i][min] = (byte) j;
		++counts[i];
	}

	private static void setB(char vectors[][], int counts[], int i, int j)
	{
		assert i >= 0;
		assert j >= 0;
		assert i < vectors.length;
		...
		{
			final int newCapacity = (int) (1 + 1.5 * counts[i]);
			final char newVector[] = new char[newCapacity];
			System.arraycopy(vectors[i], 0, newVector, 0, min);
			System.arraycopy(vectors[i], min, newVector, min + 1, counts[i] - min);
			vectors[i] = newVector;
		}
	...
	private static Iterable <Entry> iterableA(byte vectors[][], int i, final int count)
	{
		assert i >= 0 && i < vectors.length;
		final byte vector[] = vectors[i];
		if (vector == null)
		{
			assert count == 0;
	...
	private static Iterable <Entry> iterableB(char vectors[][], int i, final int count)
	{
		assert i >= 0 && i < vectors.length;
		final char vector[] = vectors[i];
		if (vector == null)
		{
			assert count == 0;
			return Collections.emptyList();
	...
	private static double getA(char vector[], int count, int i)
	{
		assert i >= 0;
	...
	private static double getB(byte vector[], int count, int i)
	{
		assert i >= 0;
