...
final class MemoryBinaryMatrixCharInt
    implements Matrix
{
	public MemoryBinaryMatrixCharInt(int m, int n)
	{
		rows = new int[m][];
		cols = new char[n][];
		rowCounts = new int[m];
	...
	private static void setA(int vectors[][], int counts[], int i, int j)
	{
		assert i >= 0;
		assert j >= 0;
	...
	private static Iterable <Entry> iterableA(int vectors[][], int i, final int count)
	{
	...
	private static double getB(int vector[], int count, int i)
	{
		assert i >= 0;
