...
final class MemoryBinaryMatrixIntChar
    implements Matrix
{
	public MemoryBinaryMatrixIntChar(int m, int n)
	{
		rows = new char[m][];
		cols = new int[n][];
		rowCounts = new int[m];
	...
	private static void setB(int vectors[][], int counts[], int i, int j)
	{
		assert i >= 0;
		assert j >= 0;
		assert i < vectors.length;
		...
		{
			final int newCapacity = (int) (1 + 1.5 * counts[i]);
			final int newVector[] = new int[newCapacity];
			System.arraycopy(vectors[i], 0, newVector, 0, min);
			System.arraycopy(vectors[i], min, newVector, min + 1, counts[i] - min);
			vectors[i] = newVector;
		}
	...
	private static Iterable <Entry> iterableB(int vectors[][], int i, final int count)
	{
		assert i >= 0 && i < vectors.length;
		final int vector[] = vectors[i];
		if (vector == null)
		{
			assert count == 0;
			return Collections.emptyList();
	...
	private static double getA(int vector[], int count, int i)
	{
		assert i >= 0;
